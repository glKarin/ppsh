...
void Node::undefer () {
    if (state == state_deferred) {
        ...
        warningLog () << nodeName () << " call on not deferred element" << endl;
}
...
KDE_NO_EXPORT void Node::insertBefore (QString &xml, Node *b) {
    Node *last = lastChild ();
    QTextStream inxml (&xml, QIODevice::ReadOnly);
    readXML (this, inxml, QString(), false);
    if (lastChild () != last) {
        NodePtr n = lastChild ();
        n->normalize ();
        if (b) {
...
Element::Element (NodePtr &doc, short id)
    : Node (doc, id), d (NULL) {}
...
static bool hasMrlChildren (const NodePtr & e) {
    for (Node *c = e->firstChild (); c; c = c->nextSibling ())
        if (id_node_ref_url != c->id &&
                (c->isPlayable () || hasMrlChildren (c)))
            return true;
    return false;
}
    ...
    if (!path.isEmpty() && !path.startsWith ("tv:/")) {
        for (Node *e = parentNode (); e; e = e->parentNode ()) {
            ...
            if (mrl && !mrl->src.isEmpty () && mrl->src != src
                    && !mrl->src.startsWith ("Playlist:/")) {
                path = QUrl (mrl->absolutePath ()).resolved (QUrl (src)).toString ();
                break;
            }
        }
    }
...
void Mrl::begin () {
    debugLog () << nodeName () << src << this << endl << endl;
    if (!src.isEmpty ()) {
        if (!media_info)
            media_info = new MediaInfo (this, MediaManager::AudioVideo);
        if (!media_info->media)
...
void Mrl::parseParam (const TrieString & para, const QString & val) {
    if (para == Ids::attr_src && !src.startsWith ("#")) {
        ...
        if (abs != src)
            ...
            src = QUrl (abs).resolved (QUrl (val)).toString ();
...
QString *Mrl::parsePanZoomString (const QString &pz, QString *coord) {
    int i;
    QStringList list = pz.split (QChar (','));
...
#ifdef _HARMATTAN
Document::Document (const QString & s, const QString &a, PlayListNotify * n)
 : Mrl (dummy_element, id_node_document),
   notify_listener (n),
   m_tree_version (0),
   event_queue (NULL),
...
void Document::cancelPosting (Posting *e) {
    if (cur_event && cur_event->event == e) {
        ...
        if (ed) {
            ...
            errorLog () << "Posting not found" << endl;
        }
    }
}
...
void Document::pausePosting (Posting *e) {
    if (cur_event && cur_event->event == e) {
        ...
        if (ed) {
            ...
            errorLog () << "pauseEvent not found" << endl;
        }
    }
}
...
#ifdef _HARMATTAN
GenericURL::GenericURL (NodePtr & d, const QString & s, const QString &a, const QString & name)
 : Mrl (d, id_node_playlist_item) {
    src = s;
    if (!src.isEmpty ())
        setAttribute (Ids::attr_src, src);
		...
		if(!audio.isEmpty())
			setAttribute(Ids::attr_audio, audio);
...
CacheAllocator *KMPlayer::shared_data_cache_allocator = NULL;
...
bool DocumentBuilder::startTag(const QString &tag, const AttributeList &attr) {
    if (m_ignore_depth) {
        ...
        if (!n) {
            debugLog () << "Warning: unknown tag " << tag << endl;
...
bool DocumentBuilder::endTag (const QString & tag) {
    if (m_ignore_depth) { // endtag to ignore
        m_ignore_depth--;
        debugLog () << "Warning: ignored end tag " << " ignore depth = " << m_ignore_depth << endl;
    } else if (!m_node) {
        return false; // had underflow
    } else {  // endtag
        NodePtr n = m_node;
        while (n) {
            ...
            if (n == m_root) {
                ...
                warningLog () << "endtag: no match " << tag << endl;
                break;
            } else
